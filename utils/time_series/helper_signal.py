# Base class for signal sampler
# Code is copied from:
# https://github.com/TimeSynth/TimeSynth/blob/master/timesynth/signals/base_signal.py
# https://github.com/TimeSynth/TimeSynth/blob/master/timesynth/signals/gaussian_process.py
...
class GaussianProcess(BaseSignal):
    ...
    def __init__(self, kernel="SE", lengthscale=1., mean=0., variance=1., c=1., gamma=1., alpha=1., offset=0.,
                 nu=5. / 2, p=1.):
    ...
    def sample_vectorized(self, time_vector):
        ...
        covariance_matrix[
            np.diag_indices_from(covariance_matrix)] += 1e-12  # Add small value to diagonal for numerical stability
        return np.random.multivariate_normal(mean=np.full(shape=(time_vector.shape[0],), fill_value=self.mean),
